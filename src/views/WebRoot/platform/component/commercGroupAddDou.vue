<template>
	<el-container class="app-container">
		<el-header>
			<el-card>
				<el-form :inline="true" :model="form" class="demo-form-inline">
					<el-form-item label="">
						<el-input v-model="form.searchContent" placeholder="请输入商户名称或商户编码"></el-input>
					</el-form-item>
					<el-form-item>
		...
		<el-main>
			<el-card>
				<el-table ref="tableData" :data="tableData" v-loading="tableLoading" border highlight-current-row @selection-change="handleSelectionChange">
...
<script>
	import { selectSellerInfoJust } from '@/api/platform/commercRegister';
	// 获取vuex
			...
			handleAdd() {
				...
				if(this.multipleSelection.length==0) {
					...
					this.multipleSelection.forEach((val,index)=>{
						sellerId+=val.codeCommerc+',';
						sellerMan+=val.nameCommerc+',';
					})
					sellerId=sellerId.substr(0,sellerId.length-1);
					sellerMan=sellerMan.substr(0,sellerMan.length-1);
					let params = {
						sellerId: sellerId,
						sellerMan: sellerMan
					}
