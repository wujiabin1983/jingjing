...
<template>
	<el-container class="app-container">
		<el-main>
			<el-tree :props="defaultProps" ref="tree" :load="loadNode" @node-click="handleCheckClick" lazy :expand-on-click-node="false" default-expand-all></el-tree>
			<!-- default-expand-all -->
		</el-main>
		<el-footer>
...
<script>
	...
	export default {
		...
		methods: {
			...
			handleCheckClick(data, node, self) {
				//      	if(node.parent.parent){
				//              console.log(node.parent.parent.data.departmendIdFull);
				//      	}
				this.selectedTreeNode = {
					departmentId: data.departmendIdFull,
					departmentName: data.label,
				}
				//          console.log(JSON.stringify(this.selectedTreeNode));
			},
			// 确定
			handleSubmit() {
				this.$emit('childSubmit', this.selectedTreeNode);
			}
		},
		computed: {
			...mapGetters([
