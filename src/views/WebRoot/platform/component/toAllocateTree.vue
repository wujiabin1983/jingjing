<template>
	<el-container class="app-container" v-loading="tableLoading">
		...
		<el-footer>
			<el-button type="primary" class="flaotLeft marginTop10" @click="handleSubmit" v-if="pageType == '查看' ? false : true">确定</el-button>
		</el-footer>
	</el-container>
</template>
<script>
	...
	export default {
		data() {
			return {
				...
				checkedKey: [], // 选中的节点
				checkedAll: [], // 用于返回所有勾选节点

				isButtonShow: false, // 是否显示按钮

                treeLevel: {} // 遍历的节点等级划分数据
			}
		},
		props: ['selected', 'pageType'],
		methods: {
            handleOpenTree() {
						...
						if(code == 'SUCCESS') {
							...
							if(res.length > 0) {
                                ...
                                res.forEach((val, index) => {
                                    if(!val.leaf) {
                                        that.treeLevel[node.level].push(val.id);
                                    }
                                    // 只展开前两级
                                    if(node.level < 2) {
                                        that.defaultExpendedKeys.push(val.id);
                                    }
                                });
								...
								res.forEach((val, index) => {
									...
									that.checkedAll = that.$refs.tree.getCheckedAll();
								});
							}
							resolve(treeData);
						} else {
...
//						console.log(err);
			...
			handleCheckChange(data, checked, indeterminate) {
				...
				// console.log('****************************');
				this.checkedAll = this.$refs.tree.getCheckedAll();
				if(checked) { // 节点本身被选中
					// console.log(data);
...
//						console.log(err);
		...
		created() {
            this.updateSelected();
            // console.log(this.returnSelected, 'toAllocatetree');
			this.isShow = true;
		},
		watch: {
