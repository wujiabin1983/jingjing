		...
		<el-card>
			<el-form :model="form" ref="form" :rules="this.getPageType=='查看' ? norules : rules" label-width="120px" class="demo-form">
				<el-form-item label="执行动作" prop="implementAction">
					...
					<el-select v-model="form.implementAction" placeholder="请选择执行动作" :disabled="giveTypeDis" v-else @change="implementAction">
				...
				<!--</el-form-item prop="implementType">-->
				<el-form-item label="执行计划">
					<el-input v-model="form.implementType" :disabled="disabled" v-if="this.getPageType=='查看'"></el-input>
					<el-select v-model="form.implementType" placeholder="请选择执行计划" @change="planInfo" v-else>
						...
						<el-option label="活动开始时间" value="活动开始时间" v-if="otherDateDis"></el-option>
				...
				<el-form-item label="执行日期" v-if="flag=='指定时间'" prop="implementTime">
					...
					<!--选择时间-->
					<div v-else>
						<el-date-picker v-model="selectDate" :picker-options="pickerOptions0" :editable="false" type="date" placeholder="选择日期" format="yyyy 年 MM 月 dd 日" @change="assignDate" value-format="yyyy-MM-dd"></el-date-picker>
						<!--<el-time-picker arrow-control v-model="selectTime" format="HH ： mm ： ss " value-format="HH:mm:ss" @change="assignTime" :picker-options="{ selectableRange: '0:00:00 - 23:59:59' }" placeholder="请选择"></el-time-picker>-->
					</div>
				</el-form-item>
				<el-form-item label="周期选择" v-if="flag=='每周'" prop="implementTime">
					<el-input v-model="sendDate" :disabled="disabled" v-if="this.getPageType=='查看'"></el-input>
					<el-select v-model="implementTime" multiple placeholder="请选择周期选择" v-else @change="weekChange">
				...
				<el-form-item label="执行日期" v-if="flag=='活动开始时间'" prop="implementTime">
					<el-input v-model="otherDate" :disabled="true"></el-input>
				</el-form-item>
				...
				<el-form-item label="执行条件" v-if="this.index!=0">
					...
					<el-radio-group v-model="implement" @change="implementChange" v-else>
						<el-radio label="无条件"></el-radio>
						<el-radio label="自定义条件">
							<el-select v-model="formImplement" multiple filterable placeholder="请选择执行条件" :disabled="disabledImplement" @change="implementSelect">
				...
				<el-form-item label="执行时间" prop="sendDate" v-if="flag!='自动执行'">
					...
					<!--<el-date-picker v-model="transmissionTime" format="yyyy 年 MM 月 dd 日" value-format="yyyy-MM-dd" v-else @change="timeChange" type="daterange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期"></el-date-picker>-->
				</el-form-item>
				<el-form-item label="发送渠道">
					<el-radio-group v-model="form.sendChannel" class="sendChannel">
					...
					<el-card class="width50"  v-if="form.sendChannel=='短信发送'">
						<el-form :model="form" label-width="120px">
							...
							<el-form-item label="消息内容">
								<el-input type="textarea" :rows="2" readonly="readonly" resize='none' :disabled="disabled"  v-model="form.messageContent" :autosize="{ minRows: 4, maxRows: 6}"></el-input>
							</el-form-item>
						</el-form>
					</el-card>
				</el-form-item>
				<el-form-item label="抄送对象">
					<el-input v-model="carbonCopyObj" :disabled="disabled" v-if="this.getPageType=='查看'"></el-input>
...
//					"implementMode": "单次手动", //执行方式
				...
				rules: {
					...
					//              ],
					//              carbonCopyObj:[
					//                  { required: true, message: '请选择抄送对象', trigger: 'change' },
					//                  { max: 2000, message: '最小长度为2000', trigger: 'change' }      
					//              ]
				},
				implementData: [{
					index: "0",
...
//					selectTime: this.selectTime,
			...
			timeChange(val) {
				...
				this.form.sendDate = JSON.stringify(date);
			},
			//选择周  每周  周
			weekChange(val) {
				//  		console.log(val)
...
//			console.log(giveTypeInfo)
			...
			if(this.getPageType == '查看') {
				...
				let res = this.data;
				//          console.log(JSON.stringify(res)+"fanhui")
				...
				if(this.planType == "每周") {
					...
					if(this.form.implementTime != "") {
						...
						this.implementTime = this.form.cycleSelect.selectTime;
						this.form.cycleSelect = JSON.stringify(params);
						this.form.implementTime = JSON.stringify(params);
					}
				}
