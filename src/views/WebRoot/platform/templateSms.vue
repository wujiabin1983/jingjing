...
<template>
	<el-container class="app-container">
		<el-header>
			<el-card v-if="roleBtn.addSmsTemplateInfo">
				...
				<el-table ref="tableData" :data="tableData" @selection-change="handleSelectionChange" v-loading="tableLoading">
					...
					<el-table-column prop="action" label="操作">
						<template slot-scope="scope">
							...
							<el-tooltip class="item" content="修改" placement="top" v-if="roleBtn.updateSmsTemplateInfo && !scope.row.isEdit">
								<!-- 修改 -->
								<icon-svg icon-class="xiugai" id="icon-xiugai" @click.native.prevent="iconXiugai(scope.$index, scope.row)" />
							</el-tooltip>
							<el-tooltip class="item" content="删除" placement="top" v-if="roleBtn.deleteSmsTemplateInfo">
								<!-- 删除 -->
	...
	export default {
		data() {
			return {
				// 权限
				roleBtn: {
					selectSmsTemplateInfo: false,
					addSmsTemplateInfo: false,
					updateSmsTemplateInfo: false,
					deleteSmsTemplateInfo: false
				},
				// 表格数据
				tableData: [],
				// 表格多选
