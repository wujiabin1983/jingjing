...
<template>
	<el-container class="app-container">
		<el-header>
			<el-card v-if="roleBtn.addGroupTemplateInfo || roleBtn.selectGroupTemplateInfo">
				<el-form :inline="true" :model="form" class="demo-form-inline" v-if="roleBtn.selectGroupTemplateInfo">
					<el-form-item class="floatLeft">
						<el-button type="primary" @click="addTable" v-if="roleBtn.addGroupTemplateInfo">新建分组</el-button>
					</el-form-item>
				...
				<el-table ref="tableData" :data="tableData" @selection-change="handleSelectionChange" v-loading="tableLoading">
					...
					<el-table-column prop="action" label="操作">
						<template slot-scope="scope">
							...
							<el-tooltip class="item" content="修改" placement="top" v-if="roleBtn.updateGroupTemplateInfo && !scope.row.isEdit">
								<!-- 修改 -->
								<icon-svg icon-class="xiugai" id="icon-xiugai" @click.native.prevent="iconXiugai(scope.$index, scope.row)" />
							</el-tooltip>
							<el-tooltip class="item" content="确认" placement="top" v-if="roleBtn.updateGroupTemplateInfo && scope.row.isEdit">
								<!-- 确认 -->
								<icon-svg icon-class="queren" id="icon-queren" @click.native.prevent="iconQueren(scope.$index, scope.row)" />
							</el-tooltip>
							<el-tooltip class="item" content="删除" placement="top" v-if="roleBtn.deleteGroupTemplateInfo">
								<!-- 删除 -->
	...
	export default {
		data() {
			return {
				// 权限
				roleBtn: {
					selectGroupTemplateInfo: false,
					addGroupTemplateInfo: false,
					updateGroupTemplateInfo: false,
					deleteGroupTemplateInfo: false
				},
				// 搜索数据
				form: {
					groupName: '' // 分组名称
		...
		methods: {
			...
			// 方法
			onSubmit() {
				this.showTable('', this.limit, this.form.groupName);
			},
