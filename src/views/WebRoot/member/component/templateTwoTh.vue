...
<template>
	...
	<el-form :model="form"  v-loading="this.uploadLoading" ref="form" label-width="120px" class="demo-form">
		<el-form-item label="选择门店" v-if="this.getPageType!='查看'" placeholder="请选择门店" prop="relationObj">
		...
		<el-form-item label="门店" v-else>
			<el-table ref="tableData" :data="storeData" class="storeTable" stripe>
				<el-table-column type="index" width="50" label="序号"></el-table-column>
				<el-table-column prop="storeName" label="门店名称"></el-table-column>
			</el-table>
		</el-form-item>
		<el-form-item label="活动对象" prop="data">
			<el-radio-group v-model="activeObject" @change="radioActiveObjectHandle" class="radioSex">
				<el-radio label="预置分组" :disabled="disabled" class="radioGroup">预置分组</el-radio>
				<el-radio label="会员分组" :disabled="disabled" class="radioGroup">会员分组</el-radio>
				<el-radio label="自定义条件" :disabled="disabled" class="radioGroup">自定义条件</el-radio>
				<el-radio label="选择用户" :disabled="disabled" class="radioGroup">选择用户</el-radio>
			</el-radio-group>
		</el-form-item>
		<el-col class="activeObj">
			<el-card v-show="activeObject == '预置分组'">
				...
				<!-- 表格 -->
				<el-table ref="multipleTable" v-loading="" v-if="this.getPageType!='查看'" :data="tableDataPreset"  border @selection-change="handleCurrentChange">
					<el-table-column type="index" width="50" label="序号"></el-table-column>
					<el-table-column type="selection" width="50" label="选择"></el-table-column>
					<el-table-column prop="groupName" label="分组名称"></el-table-column>
				</el-table>
				<el-table ref="multipleTable" v-else :data="tableDataPresetInfo" border stripe>
					<el-table-column type="index" width="50" label="序号"></el-table-column>
					<el-table-column prop="groupName" label="分组名称"></el-table-column>
				</el-table>
				<!-- 分页 -->
			...
			<el-card v-show="activeObject == '会员分组'">
				<el-form :inline="true" :model="group" class="demo-form-inline" v-if="this.getPageType!='查看'">
					<el-form-item>
						<el-input v-model="group.groupName" placeholder="请输入分组名称"></el-input>
					</el-form-item>
					<el-form-item>
						<el-button type="primary" @click="searchGroup">查询</el-button>
						<el-button type="primary" @click="clickSelet1" v-show id="returnData1">回显</el-button>
					</el-form-item>
				</el-form>
				<!-- 表格 -->
				<el-table ref="tableDataPresetUser" :data="tableDataPresetUser" v-if="this.getPageType!='查看'" v-loading="this.groupLoading"   border @selection-change="handleCurrentUserChange">
					<el-table-column type="index" width="50" label="序号"></el-table-column>
					<el-table-column type="selection" width="50" label="选择"></el-table-column>
					<el-table-column prop="groupName" label="分组名称"></el-table-column>
				</el-table>
			...
			<el-card v-show="activeObject == '自定义条件'">
				<el-form :model="customConditionForm"  ref="customConditionForm" label-width="110px">
					...
					<el-form-item label="会员消费" prop="memberConsume">
						<el-radio-group v-model="consumption" @change="consumptionHandle">
							<el-form :inline="true" class="demo-form-inline padding0">
								...
								<el-input v-model="memberConsume.operator" :disabled="true" class="memberConsumeOperator" v-if="this.getPageType=='查看'"></el-input>
							...
							<el-form :inline="true" class="demo-form-inline">
								...
								<el-input v-model="memberConsume1.operator" :disabled="true" v-if="this.getPageType=='查看'" class="memberConsumeOperator"></el-input>
								<el-form-item label="" v-else>
			...
			<el-card v-show="activeObject == '选择用户'">
				<el-form :inline="true" :model="selectUserForm" class="demo-form-inline mrgBottom10" >
					<el-button type="primary" @click="upload" v-if="this.getPageType!='查看'">选择文件导入</el-button>
					<el-button type="primary" @click="download" v-if="this.getPageType!='查看'">下载模板</el-button>
				</el-form>
				<el-table ref="tableDataUser" :data="tableDataUser" stripe>
					<el-table-column type="index" width="50" label="序号"></el-table-column>
...
<script>
	...
	import { selectProCityArea, selectStoreData, selectGroupInfo, selectTabData,selectLevalInfo,selectUserGroupName } from '@/api/public'
...
//							                      console.log(JSON.stringify(data)+"会员等级")
				...
				if(str == "会员分组"){
					this.showGroupTable(this.pageGroup, this.limit);
				}
			},
			// 会员消费单选选中事件
			...
			// 用户姓名搜索
			searchUserHandle() {
				let msg = this.selectUserForm.name;
				//          console.log(msg);
			},
			// 显示表格 预置分组
...
//			            console.log(JSON.stringify(data)+"返回值")
						...
						that.tableDataPresetUser = result;
						that.groupLoading = false;
						if(that.getPageType != "查看") {
							that.$nextTick(function(){
								document.getElementById('returnData1').dispatchEvent(new MouseEvent('click'));
							})
						}
					})
					.catch(function(err) {
...
//						console.log(err);
			...
			clickSelet1(){
				...
				if(that.multipleSelectionUser.length>0){
					...
					array.forEach(row => {
			            that.$refs.tableDataPresetUser.toggleRowSelection(row);
		            });
				}
			},
			back() { //上一步
			...
			next() { //下一步
				...
				if(this.radiusType == "自定义条件") {
					if(this.consumption == "单笔消费") {
						//						console.log(this.memberConsume.operator+"[]"+this.memberConsume.money)
...
//							console.log(JSON.stringify(params))
		...
		created() {
			...
			if(sessionStorage.getItem("dataTwo")) {
				...
				//回显数据
				if(JSON.parse(data).relationObj){
					this.form.relationObj = JSON.parse(data).relationObj;
					//门店数据回显
					this.value2 = this.form.relationId;
					this.form.relationObj = {
						relationId:JSON.parse(data).relationId,
						relationName:JSON.parse(data).relationName
					};
					this.storeArr = JSON.parse(JSON.parse(data)).relationName;
				}
				//根据提交的活动对象类型回显
//				console.log(JSON.stringify(data))
			...
			if(this.getPageType == '查看') {
				...
				if(res.activeTempMas.relationObj!=""){
					this.form = {
						relationObj: res.activeTempMas.relationObj // 适用门店
					}
					let str = res.activeTempMas.relationObj.relationName;
					let array = str.split(",");
				...
				if(res.activeTempMas.activeObjectInfo!=""){
					this.form.activeObjectInfo= res.activeTempMas.activeObjectInfo;
				}
				...
				if(res.chooseUser != "") {
					...
					this.form.activeObjectInfo = JSON.stringify(msg); //选择用户参数
					this.masterId= JSON.stringify(msg); //选择用户参数
				}
			} else if(this.getPageType == '修改' ||this.getPageType == '复制'&& !sessionStorage.getItem("dataTwo")) {
				let res = JSON.parse(this.getFormData);
//				console.log(res.activeTempMas.relationObj)
				if(res.activeTempMas.relationObj!=""){
					this.form = {
						relationObj: res.activeTempMas.relationObj // 适用门店
					}
					this.value2 = res.activeTempMas.relationObj.relationId;
					//回显选择门店input值
					this.storeArr = res.activeTempMas.relationObj.relationName;
					this.form.relationObj = JSON.stringify(res.activeTempMas.relationObj);
				}
				res = res.activeTempMas;
				if(res.presetGroup != "") {
					this.activeObject = "预置分组";
