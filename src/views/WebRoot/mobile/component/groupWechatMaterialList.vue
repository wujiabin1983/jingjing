	...
	<el-container>
		...
		<el-main class="overHidden" v-loading="elMainLoading">
			<el-row class="msg-list">
				<el-col :span="7" v-for="(item, index) in items" :key="index">
					<div class="component">
						<div :is="item.component" :childDataLists="dataLists[index]" @parentMasterId="handleMasterId" :childChecked="checked" :isCheckBox="true" ref="groupWechatMaterialListShow"></div>
					</div>
				</el-col>
			</el-row>
			<!-- 分页 -->
				...
				MaterialLibraryFiveShow(params)
					.then((res) => {
						...
						msg.forEach((val, index) => {
							that.items.push({
								'id': length++,
								'component': 'group-Wechat-material-list-show'
							});
						});
						if(that.$refs.groupWechatMaterialListShow.length > 0) {
							that.$refs.groupWechatMaterialListShow.forEach((val, index) => {
...
//						 console.log(err);
			...
			handleMasterId(masterId) { // 传递masterId
				let data = {};
				if(this.$refs.groupWechatMaterialListShow.length > 0) {
					this.$refs.groupWechatMaterialListShow.forEach((val, index) => {
