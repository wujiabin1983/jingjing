  ...
  <el-card class="centerInfo add-member-return-task-wrap">
    ...
    <el-form :model="form" v-loading="loading" :rules="rules" ref="form" label-width="120px" class="from-wrap">
      ...
      <el-form-item label="会员分组" prop="storeGroupName">
        ...
        <el-button type="primary" @click="openSelectGroup" class="mt">选择分组</el-button>
      </el-form-item>
      <el-form-item label="组织选择" prop="storeGroupDesc">
        ...
        <el-button type="primary" @click="openStore" class="mt">选择门店</el-button>
      </el-form-item>
      ...
      <el-form-item label="会员等级" prop="levalId">
      ...
      <el-form-item label="消费类型">
        <el-radio-group v-model="form.consumeType" @change="handleChangeConsumeType">
    ...
    <SelectGroup :isShowDialog.sync="isShowSelectGroup" :selectGroupInfo.sync="selectGroupInfo" />
    <SelectStore :isShowDialog.sync="isShowSelectStore" :selectStoreInfo.sync="selectStoreInfo" />
  </el-card>
</template>

<script>
  import {
    apiAddMemberReturnTask,
    apiUpdateMemberReturnTask,
    apiGetMemberReturnTaskDetails
  } from '@/api/work/memberReturnVisit.js'
    ...
    methods: {
      ...
      getDetails() {
        apiGetMemberReturnTaskDetails({
          ...
          let revisitTask = messageContent.revisitTask
          if (messageType == 'SUCCESS') {
            let {
              ...
              birthdayDay,
              sales
            } = revisitTask.customConditions
  
            revisitTask.taskDateArray = [taskDate.taskDateBegin, taskDate.taskDateEnd]
            revisitTask.birthdayDayArray = [birthdayDay.fromDate, birthdayDay.toDate]
            revisitTask.saleDateArray = [sales.fromSaleDate, sales.toSaleDate]
            this.selectGroupInfo = memberGroup
            this.selectStoreInfo = relationObj
            revisitTask.consumeType = sales.fromSaleNumbers && sales.fromSaleNumbers != '' ? 0 : 1
            this.form = revisitTask
      ...
      handleChangeConsumeType(val) {
        console.log(val)
        if (val == 1) {
          this.form.customConditions.sales.fromSaleNumbers = ''
          this.form.customConditions.sales.toSaleNumbers = ''
        }
      },
      submit() {
      ...
      update() {
        let params = {
          ...
          revisitTask: this.form,
  
        }
        apiUpdateMemberReturnTask(params).then((res) => {
          this.loading = false;
