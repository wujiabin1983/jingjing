...
<template>
	<el-container class="app-container">
		<el-header>
			<el-card>
				<el-form :inline="true" :model="form" class="demo-form-inline" v-if="roleBtn.selectIntegralImportInfo">
					<el-form-item class="floatLeft">
						<el-button type="primary" @click="upload" v-if="roleBtn.importIntegralInfo">选择文件导入</el-button>
						<el-button type="primary" @click="download">下载模板</el-button>
					</el-form-item>
					<div class="floatRight">
						<el-form-item label="选择日期">
							<el-date-picker v-model="form.selectDate" :editable="false" format="yyyy 年 MM 月 dd 日" type="daterange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期">
							</el-date-picker>
						</el-form-item>
						<el-form-item>
...
<script>
	...
	import { memberLabelImportInfo, memberLabelImportDownload, memberLabelImport } from '@/api/base/importInfo'
	...
	export default {
		data() {
			return {
				// 权限
				roleBtn: {
					selectIntegralImportInfo: false,
					importIntegralInfo: false
				},
				// 搜索数据
				form: {
					selectDate: [] // 日期区间
				...
				//				}
				userCode: '',
				uploadAction: Cookie.get('baseUrl') + '/I_SCRM_INTERFACE_617.action', // 导入路径
				// 表格多选
				multipleSelection: [],
				// 表格是否为加载状态
				tableLoading: false,
		...
		methods: {
			...
			//下载模板
			download() {
				//				console.log(Cookie.get('baseUrl'));
				memberLabelImportDownload()
					.then((res) => {
						let data = JSON.parse(Base64.decode(res.data));
