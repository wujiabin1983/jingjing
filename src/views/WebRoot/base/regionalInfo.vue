...
<template>
	<el-container class="app-container">
		<el-header>
			<el-card>
				<el-form :inline="true" :model="form" class="demo-form-inline" v-if="roleBtn.selectAreaImportInfo">
					<el-form-item class="floatLeft">
						<el-button type="primary" @click="upload" v-if="roleBtn.importStoresAreaInfo">选择文件导入</el-button>
...
<script>
	...
	import { regionalImportInfo, regionalImportDownload, regionalImportFile } from '@/api/base/importInfo'
	...
	export default {
		data() {
			return {
				// 权限
				roleBtn: {
					selectAreaImportInfo: false,
					importStoresAreaInfo: false
				},
				// 搜索数据
				form: {
					selectDate: [] // 日期区间
		...
		methods: {
			...
			showTable(start, limit) {
				...
				// 查询接口
				regionalImportInfo(params)
					.then(function(res) {
						let data = JSON.parse(Base64.decode(res.data));
						//						console.log(JSON.stringify(data)+"返回值1");
						let result = [];
						if(data.data) {
