...
<template>
	<el-container class="app-container">
		<el-header>
			<el-card>
				<el-form :inline="true" :model="form" class="demo-form-inline" v-if="roleBtn.selectOrderImportInfo">
					<el-form-item class="floatLeft">
						<el-button type="primary" @click="upload" v-if="roleBtn.importOrderlInfo">选择文件导入</el-button>
...
<script>
	...
	import { orderImportInfo, orderImportDownload, orderImportFile } from '@/api/base/importInfo'
	...
	export default {
		data() {
			return {
				// 权限
				roleBtn: {
					selectOrderImportInfo: false,
					importOrderlInfo: false
				},
				// 搜索数据
				form: {
					selectDate: [] // 日期区间
				...
				multipleSelection: [],
				userCode: '',
				uploadAction: Cookie.get('baseUrl') + '/I_SCRM_INTERFACE_117.action', // 导入路径
				// 表格是否为加载状态
				tableLoading: false,
				// 分页
				page: 0, // 当前页
		...
		methods: {
			...
			showTable(start, limit) {
				...
				// 查询接口
				orderImportInfo(params)
					.then(function(res) {
						let data = JSON.parse(Base64.decode(res.data));
						//						console.log(JSO/N.stringify(data)+"返回值");
						let result = [];
						if(data.data) {
