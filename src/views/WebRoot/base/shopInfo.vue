...
<template>
	<el-container class="app-container">
		<el-header>
			<el-card>
				<el-form :inline="true" :model="form" class="demo-form-inline" v-if="roleBtn.selectStoresImportInfo">
					<el-form-item class="floatLeft">
						<el-button type="primary" @click="upload" v-if="roleBtn.importStoresInfo">选择文件导入</el-button>
...
<script>
	/**
	 * shopImportInfo - 显示店铺导入数据
	 * shopImportDownload - 下载店铺信息导入模板
	 * shopImportFile - 店铺信息文件导入
	 */
	import { permission } from '@/utils'
	import { shopImportInfo, shopImportDownload, shopImportFile } from '@/api/base/importInfo'
	...
	export default {
		data() {
			return {
				// 权限
				roleBtn: {
					selectStoresImportInfo: false,
					importStoresInfo: false
				},
				// 搜索数据
				form: {
					selectDate: [] // 日期区间
				...
				userCode: '',
				uploadAction: Cookie.get('baseUrl') + '/I_SCRM_INTERFACE_502.action', // 导入路径
				// 表格数据
				tableData: [{
					"id": "",
					"version": "",
...
//												console.log(data)
			...
			preview(file) {
				let res = Base64.decode(file.response);
				//				console.log(res);
			},
			//下载模板
			download() {
