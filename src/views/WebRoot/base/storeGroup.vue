...
<template>
	<el-container class="app-container">
		<el-header>
			<el-card>
				<el-form :inline="true" :model="form" class="demo-form-inline" v-if="roleBtn.selectGroupImportInfo">
					<el-form-item class="floatLeft">
						<el-button type="primary" @click="upload" v-if="roleBtn.importStoreGroupInfo">选择文件导入</el-button>
...
<script>
	...
	import { storeGroupImportInfo, storeGroupImportDownload, storeGroupImportFile } from '@/api/base/importInfo'
	...
	export default {
		data() {
			return {
				// 权限
				roleBtn: {
					selectGroupImportInfo: false,
					importStoreGroupInfo: false
				},
				// 搜索数据
				form: {
					selectDate: ['', ''] // 日期区间
				...
				userCode: '',
				uploadAction: Cookie.get('baseUrl') + '/I_SCRM_INTERFACE_134.action', // 导入路径
				// 表格多选
				multipleSelection: [],
				// 表格是否为加载状态
				tableLoading: false,
		...
		methods: {
			...
			showTable(start, limit) {
				let that = this;
				let params = {
					uploadTimeBegin: this.form.selectDate[0],
					uploadTimeEnd: this.form.selectDate[1],
				...
				// 查询接口
				storeGroupImportInfo(params)
					.then(function(res) {
						let data = JSON.parse(Base64.decode(res.data));
						//						console.log(JSON.stringify(data)+"返回值查询");
						let result = [];
						if(!data.messageType) {
			...
			download() {
				...
				storeGroupImportDownload()
					.then((res) => {
						//						console.log(JSON.stringify(res)+"返回值下载")
						let data = JSON.parse(Base64.decode(res.data));
