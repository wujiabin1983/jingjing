...
<template>
	<el-container class="app-container">
		<el-header>
			<el-card>
				<el-form :inline="true" :model="form" class="demo-form-inline" v-if="roleBtn.selectMemImportInfo">
					<el-form-item class="floatLeft">
						<el-button type="primary" @click="upload" v-if="roleBtn.importMemberInfo">选择文件导入</el-button>
...
<script>
	...
	import { memberImportInfo, memberImportDownload, memberImportFile } from '@/api/base/importInfo'
	...
	export default {
		data() {
			return {
				// 权限
				roleBtn: {
					selectMemImportInfo: false,
					importMemberInfo: false
				},
				formUpload: {},
				// 搜索数据
				form: {
				...
				multipleSelection: [],
				userCode: '',
				uploadAction: Cookie.get('baseUrl') + '/I_SCRM_INTERFACE_324.action', // 导入路径
				// 表格是否为加载状态
				tableLoading: false,
				// 分页
				page: 0, // 当前页
